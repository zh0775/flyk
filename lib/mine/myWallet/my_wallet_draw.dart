...
class MyWalletDrawController extends GetxController {
  ...
  //             String title = no.length > 4 ? "${e["bankName"] ?? ""}(${no.substring(no.length - 4, no.length)})" : e["bankName"] ?? "";
  //             return {...e, "name": e["bankName"] ?? "", "no": no, "type": 1, "title": title};
  //           });
  //           accountList = [
  //             ...accountList,
  //             ...cardList,
    ...
    simpleRequest(
        ...
        success: (success, json) {
          ...
          push(
              MyWalletDrawResult(
                success: success,
                describe: success ? "提现后7个工作日内到账，若有疑问请联系客服" : json["messages"] ?? "",
  ...
  alipayAlert(Function() close) {
    showAuthAlert(context: Global.navigatorKey.currentContext!, isAuth: false, alipay: true, close: close, haveClose: false);
  }

  authAlert(Function() close) {
    showAuthAlert(context: Global.navigatorKey.currentContext!, isAuth: true, close: close, haveClose: false);
  }

  bindAlert(Function() close) {
    showAuthAlert(context: Global.navigatorKey.currentContext!, isAuth: false, close: close, haveClose: false);
  }
  ...
  drawAction() {
    ...
    if (double.parse(drawInputCtrl.text) < double.parse((walletData["minCharge"] ?? 0.0))) {
  ...
  dataFormat() {
    ...
    List payTypes = ((drawInfo["System_PayType"] ?? "") as String).split(",");

    openCard = payTypes.contains("2");
    openAlipay = payTypes.contains("1");
    ...
    if (haveAlipay && openAlipay) {
      // alipayData = {"name": authentication["user_OnlinePay_Name"], "accound": authentication["user_OnlinePay_Account"], "type": 1};
      String accName = "支付宝";
      String accNo = authentication["user_OnlinePay_Account"];
      String title = accNo.length > 4 ? "$accName(${accNo.substring(accNo.length - 4, accNo.length)})" : accName;
      // alipayData = {"name": authentication["user_OnlinePay_Name"], "accound": authentication["user_OnlinePay_Account"]};
      alipayData = {"name": accName, "no": accNo, "type": 1, "title": title};
      accountList.add(alipayData);
      accountList.add(alipayData);
    }
    if (haveCard && openCard) {
      // cardData = {"name": authentication["bank_AccountName"], "accound": authentication["bank_AccountNumber"], "type": 2};
      String accName = authentication["bank_Name"];
      String accNo = authentication["bank_AccountNumber"];
      String title = accNo.length > 4 ? "$accName(${accNo.substring(accNo.length - 4, accNo.length)})" : accName;
      // cardData = {"name": authentication["bank_AccountName"], "accound": authentication["bank_AccountNumber"]};
      cardData = {"name": accName, "no": accNo, "type": 2, "title": title};
    ...
    return GestureDetector(
        ...
        child: Scaffold(
            appBar: getDefaultAppBar(context, "提现", action: [
              CustomButton(
                  onPressed: () {
                    push(const MyWalletDrawHistory(), null, binding: MyWalletDrawHistoryBinding(), arguments: {"a_No": walletData["a_No"] ?? 0});
                  },
                  child: SizedBox(width: 80.w, height: kToolbarHeight, child: Center(child: getSimpleText("提现记录", 14, AppColor.textBlack))))
            ]),
            body: getInputBodyNoBtn(context, contentColor: AppColor.pageBackgroundColor, buttonHeight: 0, build: (boxHeight, context) {
              return SingleChildScrollView(
                  ...
                  child: Column(children: [
                    ...
                    Container(
                        ...
                        child: Column(children: [
                            ...
                            CustomButton(
                                onPressed: () {
                                  if (controller.accountList.isEmpty) {
                                    push(const DebitCardAdd(), context, binding: DebitCardAddBinding());
                                    return;
                                  }
                                ...
                                child: SizedBox(
                                    ...
                                    child: Center(
                                        child: centRow([
                                      GetX<MyWalletDrawController>(builder: (_) {
                                        return controller.accountList.isEmpty
                                            ? centRow([
                                                Image.asset(assetsName("statistics/machine/btn_navi_add"), width: 18.w, fit: BoxFit.fitWidth),
                                                gwb(3),
                                                getSimpleText("添加", 14, AppColor.textBlack),
                                                gwb(5)
                                              ])
                                            : centRow([
                                                getSimpleText(controller.accountList[controller.defaultAccountIdx]["title"], 15,
                                                    controller.accountList.isEmpty ? AppColor.text3 : AppColor.text,
                                                    isBold: true),
                                                Image.asset(assetsName("statistics/icon_arrow_right_gray"), width: 18.w, fit: BoxFit.fitWidth)
                                              ]);
                                      })
                                    ]))))
                          ...
                          SizedBox(
                              ...
                              height: 88.w,
                              child: Row(children: [
                                getWidthText("¥", 24, AppColor.text, 30, 1, textHeight: 1.0),
                                CustomInput(
                                    width: 345.w - 11.w * 2 - 30.w,
                                    heigth: 88.w,
                                    textEditCtrl: controller.drawInputCtrl,
                                    style: TextStyle(color: AppColor.text, fontSize: 24.sp, fontWeight: AppDefault.fontBold),
                                    placeholderStyle: TextStyle(color: AppColor.text3, fontSize: 16.sp, fontWeight: FontWeight.normal),
                                    placeholder: "最低提现金额${priceFormat(walletData["minCharge"] ?? 0)}元",
                    ...
                    ghb(31),
                    GetX<MyWalletDrawController>(builder: (_) {
                      return getSubmitBtn("确认提现", () {
                        controller.drawAction();
                      }, color: AppColor.theme, height: 45, enable: controller.submitEnable);
                    })
                  ]));
            })));
  }
              ...
              sbhRow([
                ...
                CustomButton(
                    ...
                    child: SizedBox(
                        width: 42.w,
                        height: 48.w,
                        child: Center(child: Image.asset(assetsName("statistics/machine/btn_model_close"), width: 20.w, fit: BoxFit.fitWidth))))
              ], width: 375, height: 48),
              ghb(5),
              gline(375, 1),
              SizedBox(
                      ...
                      child: Scrollbar(
                          child: SingleChildScrollView(
                              ...
                              child: centClm([
                                gwb(375),
                                ghb(overHeight ? 20 : 0),
                                ...List.generate(controller.accountList.length, (index) {
                                  return CustomButton(
                                      onPressed: () {
                                        controller.bottomAccountIdx = index;
                                      },
                                      child: sbhRow([
                                        getSimpleText(controller.accountList[index]["title"] ?? "", 16, const Color(0xFF333333)),
                                        GetX<MyWalletDrawController>(builder: (_) {
                                          return Image.asset(
                                              assetsName("machine/checkbox_${controller.bottomAccountIdx == index ? "selected" : "normal"}"),
                                              width: 21.w,
                                              fit: BoxFit.fitWidth);
                                        })
                ...
                children: [
                  Padding(
                    ...
                    child: sbRow([
                      centClm([
                        centRow([
                          getSimpleText("${data["name"] ?? ""}$unit", 14, Colors.white),
                          gwb(2),
                          Image.asset(
                        ...
                        ghb(10),
                        getSimpleText(priceFormat(data["amout"] ?? 0, tenThousand: tenThousand), 30, Colors.white,
                            fw: FontWeight.w700, textHeight: 1),
                        ghb(15),
                        getSimpleText("可提现金额 ￥${priceFormat(data["amout"] ?? 0, tenThousand: tenThousand)}", 12, Colors.white.withOpacity(0.5))
